...
public class MasSystemService {
    ...
    /**
     * Tạo bản ghi summaries từ kết quả MAS.
     * - content      : original_text (nếu có) hoặc userInput
     * - summaryText  : summary từ MAS
     * - grade        : grade_level từ intent (ví dụ: "5")
     * - method       : "DIEN_GIAI" cho abstractive, "TRICH_XUAT" cho extractive
     * - title        : Lấy 6 chữ đầu của văn bản gốc + "..."
     *
     * JSON tương đương với API /api/summaries:
     * {
     *   "title": "Lấy 6 chữ đầu của văn bản gốc ...",
     *   "content": "đây là content văn bản đầu vào",
     *   "summaryContent": "văn bản được tóm tắt",
     *   "createdBy": { "userId": "..." },
     *   "grade": "5",
     *   "method": "DIEN_GIAI" hoặc "TRICH_XUAT",
     *   "readCount": 0
     * }
     */
    private void createSummaryFromMas(MasChatRequest request, JsonNode responseJson) {
        try {
            ...
            try {
                if (intentRaw != null && !intentRaw.isEmpty()) {
                    ...
                    if ("extractive".equalsIgnoreCase(summarizationType)) {
                        method = "TRICH_XUAT";          // Extractive (PhoBERTSUM)
                    } else {
                        method = "DIEN_GIAI";     // Abstractive (ViT5 diễn giải)
                    }
                }
            } catch (Exception e) {
            ...
            String title = "Lấy 6 chữ đầu của văn bản gốc: " + titleCore + "...";
