...
public class MasSystemService {
    ...
    public MasChatResponse processChat(MasChatRequest request) {
        ...
        try {
            ...
            JsonNode agentConfs = responseJson.path("agent_confidences");

            // Helper log cho từng agent nếu có confidence
            logAgentIfPresent(agentConfs, "4c5cf9d4-d47a-49fc-8849-6ef18f3b5512",
                    userMessage.getMessageId(),
                    request.getUserInput(),
                    responseJson.path("plan").toString());

            logAgentIfPresent(agentConfs, "739c815d-3f28-4081-87d1-ef37415b10c5",
                    userMessage.getMessageId(),
                    responseJson.path("summary").asText(),
                    response.getFinalOutput());

            logAgentIfPresent(agentConfs, "43848fa4-d8d3-4566-a279-c0534eb56472",
                    userMessage.getMessageId(),
                    responseJson.path("summary").asText(),
                    responseJson.path("evaluation").toString());
            
            // logAgentIfPresent(agentConfs, "clarification_agent",
            //         userMessage.getMessageId(),
            //         responseJson.path("clarification_question").asText(),
            //         responseJson.path("clarification_needed").asBoolean(false) ? "true" : "false");
            
            // logAgentIfPresent(agentConfs, "extractor_agent",
            //         userMessage.getMessageId(),
            //         if 
            
            // logAgentIfPresent(agentConfs, "image2text_agent",
            //         userMessage.getMessageId(),
            //         responseJson.path("summary").asText(),
            //         responseJson.path("evaluation").toString());

            // logAgentIfPresent(agentConfs, "intent_agent",
            //         userMessage.getMessageId(),
            //         responseJson.path("summary").asText(),
            //         responseJson.path("evaluation").toString());
    ...
    private void logAgentIfPresent(JsonNode agentConfs,
                                   ...
                                   String input,
                                   String output) {
        if (agentConfs != null && agentConfs.has(agentId)) {
