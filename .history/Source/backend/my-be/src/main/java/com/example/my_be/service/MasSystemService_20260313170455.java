...
public class MasSystemService {
     ...
     * - method       : "T5_DIEN_GIAI" cho abstractive, "PHOBERT" cho extractive
     */
    private void createSummaryFromMas(MasChatRequest request, JsonNode responseJson) {
        try {
            ...
            try {
                if (intentRaw != null && !intentRaw.isEmpty()) {
                    ...
                    if ("extractive".equalsIgnoreCase(summarizationType)) {
                        method = "PHOBERT";          // Extractive (PhoBERTSUM)
                    } else {
                        method = "T5_DIEN_GIAI";     // Abstractive (ViT5 diễn giải)
                    }
                }
            } catch (Exception e) {
            ...
            // Tạo title đơn giản từ grade + preview nội dung
            String preview = originalText != null ? originalText.trim() : "";
            if (preview.length() > 60) {
                preview = preview.substring(0, 60) + "...";
            }
            String title = (grade != null ? "Tóm tắt lớp " + grade : "Tóm tắt") +
