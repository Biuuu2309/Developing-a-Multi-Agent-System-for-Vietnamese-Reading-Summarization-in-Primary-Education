...
def run_langgraph_chat_fixed(app, create_initial_state, initial_state=None):
    ...
    if not state.get("messages"):
        ...
        # Hiển thị thông tin về state để debug
        print(f"📊 Conversation stage: {state.get('conversation_stage', 'unknown')}")
        print(f"📝 Original text: {'Có' if state.get('original_text') else 'Không có'} ({len(state.get('original_text', ''))} chars)")
        print(f"📝 Processed text: {'Có' if state.get('processed_text') else 'Không có'} ({len(state.get('processed_text', ''))} chars)")
