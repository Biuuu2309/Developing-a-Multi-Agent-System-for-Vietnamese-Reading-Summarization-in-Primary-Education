...
class AbstracterAgent:
    ...
    def correct_entities(self, summary, entities):
        """
        Sử dụng danh sách thực thể để sửa lại tên riêng trong summary.
        - Ưu tiên thực thể dài (nhiều token) trước.
        - Thay thế bằng dạng có dấu cách (bỏ hết dấu gạch dưới).
        - Tránh nhân đôi thực thể khi đã xuất hiện đúng trong câu.
        """
        ...
        for entity in entities_sorted:
            ...
            entity_clean = entity.replace("_", " ").strip()
            if not entity_clean:
                continue

            # Nếu summary đã chứa đúng entity_clean thì bỏ qua để tránh lặp
            if entity_clean.lower() in clean_summary.lower():
                continue
            ...
            # Chỉ sửa khi độ giống đủ cao
            if score > 70 and match:
                clean_summary = clean_summary.replace(match, entity_clean)
    ...
    def summarize(
        ...
        if entities:
            summary = self.correct_entities(summary, entities)
