...
# Văn bản < N ký tự + lớp 1 → dùng small_model_path (nếu có)
SMALL_MODEL_CHAR_THRESHOLD = 100
SMALL_MODEL_GRADE = 1
...
class AbstracterAgent:
    ...
    def __init__(
        self,
        model_path: str = r"E:\Project_NguyenMinhVu_2211110063\Source\ai\Model Train\Model_DG_ver2\vit5_grade_summary",
    ...
    def _load_small_model(self) -> None:
        if not self.small_model_path or self._small_model is not None:
            return
        self._small_tokenizer = AutoTokenizer.from_pretrained(
            self.small_model_path, use_fast=False
        )
        self._small_model = AutoModelForSeq2SeqLM.from_pretrained(
            self.small_model_path
        )
        self._small_model.to(self.device)
        self._small_model.eval()

    def _unload_small_model(self) -> None:
        if self._small_model is None and self._small_tokenizer is None:
            return
    ...
    def _seq2seq_for_request(self, content: str, grade: int):
        if (
            ...
            and len(content.strip()) < SMALL_MODEL_CHAR_THRESHOLD
        ):
    ...
    def summarize(
        ...
        if length_option == "short":
            summary_words = int(total_words * 0.30)
        elif length_option == "long":
            summary_words = int(total_words * 0.60)
        else:
            # medium hoặc bất kỳ giá trị lạ nào → medium
            summary_words = int(total_words * 0.40)
        ...
        target_min_len = max(1, min(int(target_max_len * 0.3), target_max_len))

        # -------------------------------
        # 4–5. TOKENIZE, GENERATE, giải phóng small model khỏi GPU khi xong
        # -------------------------------
        tokenizer, seq2seq_model, used_small = self._seq2seq_for_request(
            content, grade
        )
        try:
