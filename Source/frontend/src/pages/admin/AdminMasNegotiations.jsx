...
export default function AdminMasNegotiations() {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const filtered = (Array.isArray(states) ? states : []).filter((s) => s?.negotiationResult);
      setRows(filtered);
    } catch (e) {
      setRows([]);
  ...
  return (
    <div className="admin-panel">
      <p className="admin-muted" style={{ marginBottom: 12 }}>Read-only. Du lieu lay tu mas_states (field negotiationResult).</p>
      ...
      <div className="admin-table-wrap">
        <table className="admin-table">
          ...
          <tbody>
            {rows.map((r, idx) => (
              <tr key={r.stateId || `n-${idx}`}>
                <td className="admin-cell-mono">{r.stateId || '—'}</td>
                <td className="admin-cell-mono">{r.messageId || '—'}</td>
                <td title={r.negotiationResult || ''}>{shortText(r.negotiationResult)}</td>
                <td title={r.agentConfidences || ''}>{shortText(r.agentConfidences, 70)}</td>
            ...
            {!loading && rows.length === 0 ? (
              <tr>
                <td colSpan={5} style={{ padding: 12, color: '#6b7280' }}>
                  Chua co du lieu negotiationResult.
                </td>
              </tr>
            ) : null}
          </tbody>
