...
export default function AdminMasEvaluations() {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const filtered = (Array.isArray(states) ? states : []).filter((s) => s?.evaluation);
      setRows(filtered);
    } catch (e) {
      setRows([]);
  ...
  return (
    <div className="admin-panel">
      <p className="admin-muted" style={{ marginBottom: 12 }}>Read-only. Du lieu lay tu mas_states (field evaluation).</p>
