...
export default function AdminMasGoals() {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const filtered = (Array.isArray(states) ? states : []).filter((s) => s?.goalState);
      setRows(filtered);
    } catch (e) {
      setRows([]);
  ...
  return (
    <div className="admin-panel">
      <p className="admin-muted" style={{ marginBottom: 12 }}>Read-only. Du lieu lay tu mas_states (field goalState).</p>
        ...
        <table className="admin-table">
          <thead>
            <tr>
              ...
              <th>messageId</th>
              <th>goalState</th>
              <th>semanticRecall</th>
              <th>createdAt</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, idx) => (
              <tr key={r.stateId || `g-${idx}`}>
                <td className="admin-cell-mono">{r.stateId || '—'}</td>
                <td className="admin-cell-mono">{r.messageId || '—'}</td>
                <td title={r.goalState || ''}>{shortText(r.goalState)}</td>
                <td title={r.semanticRecall || ''}>{shortText(r.semanticRecall, 70)}</td>
