...
export default function AdminMasPlans() {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const filtered = (Array.isArray(states) ? states : []).filter((s) => s?.plan);
      setRows(filtered);
    } catch (e) {
      setRows([]);
  ...
  return (
    <div className="admin-panel">
      <p className="admin-muted" style={{ marginBottom: 12 }}>Read-only. Du lieu lay tu mas_states (field plan).</p>
        ...
        <table className="admin-table">
          <thead>
            <tr>
              ...
              <th>messageId</th>
              <th>plan</th>
              <th>planRevisionCount</th>
              <th>strategyChanged</th>
              <th>createdAt</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, idx) => (
              <tr key={r.stateId || `p-${idx}`}>
                <td className="admin-cell-mono">{r.stateId || '—'}</td>
                <td className="admin-cell-mono">{r.messageId || '—'}</td>
                <td title={r.plan || ''}>{shortText(r.plan)}</td>
                <td>{r.planRevisionCount ?? 0}</td>
                <td>{r.strategyChanged ? 'true' : 'false'}</td>
