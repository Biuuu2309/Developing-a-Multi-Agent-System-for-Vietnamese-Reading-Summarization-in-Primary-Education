...
export default function AdminMasAgentMemories() {
  ...
  const handleLoad = async () => {
    ...
    try {
      ...
      const filtered = (Array.isArray(states) ? states : []).filter((s) => s?.agentMemories);
      setRows(filtered);
    } catch (e) {
      setRows([]);
  ...
  return (
    <div className="admin-panel">
      <p className="admin-muted" style={{ marginBottom: 12 }}>Read-only. Du lieu lay tu mas_states (field agentMemories).</p>
        ...
        <table className="admin-table">
          <thead>
            <tr>
              ...
              <th>messageId</th>
              <th>agentMemories</th>
              <th>knowledgeSearch</th>
              <th>toolRecommendations</th>
              <th>createdAt</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, idx) => (
              <tr key={r.stateId || `am-${idx}`}>
                <td className="admin-cell-mono">{r.stateId || '—'}</td>
                <td className="admin-cell-mono">{r.messageId || '—'}</td>
                <td title={r.agentMemories || ''}>{shortText(r.agentMemories)}</td>
                <td title={r.knowledgeSearch || ''}>{shortText(r.knowledgeSearch, 70)}</td>
                <td title={r.toolRecommendations || ''}>{shortText(r.toolRecommendations, 70)}</td>
