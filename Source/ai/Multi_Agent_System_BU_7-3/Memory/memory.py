...
class MemoryManager:
    ...
    def __init__(self):
        ...
        self.session_id_to_user_id: Dict[str, str] = {}
    ...
    def get_memory_by_session_id(self, session_id: str) -> Optional[ConversationMemory]:
        user_id = self.session_id_to_user_id.get(session_id)
        if user_id:
            return self.active_sessions.get(user_id)
        for memory in self.active_sessions.values():
            if memory.session_id == session_id:
                self.session_id_to_user_id[session_id] = memory.user_id
                return memory
        return None

    def add_message(self, role: str, content: str, user_id: str = "default_user", session_id: Optional[str] = None) -> None:
        if session_id:
    ...
    def get_context(self, user_id: str = "default_user", n: int = 5, session_id: Optional[str] = None) -> str:
        if session_id:
            memory = self.get_memory_by_session_id(session_id)
            if memory:
                return memory.get_context_string(n)
    ...
    def start_new_session(
        ...
        if session_id:
            ...
            self.session_id_to_user_id[session_id] = user_id
            self.current_session_id = session_id
            return session_id
        else:
            memory = self.get_memory(user_id)
            memory.clear(keep_preferences=keep_preferences)
            self.session_id_to_user_id[memory.session_id] = user_id
            self.current_session_id = memory.session_id
            return memory.session_id

    def get_recent_history(self, session_id: Optional[str] = None, user_id: str = "default_user", limit: int = 20) -> List[Dict[str, Any]]:
        if session_id:
            memory = self.get_memory_by_session_id(session_id)
            if memory:
                return memory.get_recent_history(limit)
        memory = self.get_memory(user_id)
        return memory.get_recent_history(limit)
