...
Text characteristics: {text_characteristics}""")
    ...
    def revise_plan(
        ...
        """
        # Tạo bản sao của plan để không modify trực tiếp
        revised_plan = json.loads(json.dumps(current_plan))
        
        # Nếu đang trong improvement mode (self-improvement loop)
        if feedback.get("improvement_mode", False):
            # Trong improvement mode, giữ nguyên strategy abstractive
            # nhưng điều chỉnh các parameters khác
            for step in revised_plan.get("pipeline", []):
                if step.get("step") == "summarize":
                    step["improvement_attempt"] = True
                    step["improvement_reason"] = "Self-improvement loop: Điều chỉnh parameters để cải thiện chất lượng"
                    # Grade level sẽ được điều chỉnh ở planning_node
                    break
        ...
        if feedback.get("needs_refinement", False):
            # Kiểm tra xem đã có refine step chưa
            has_refine = any(step.get("step") == "refine" for step in revised_plan.get("pipeline", []))
            if not has_refine:
                revised_plan["pipeline"].append({
                    "step": "refine",
